...
export class RiskManager {
  ...
  checkTrade(usdcAmount: number): RiskCheck {
    ...
    // F8: Drawdown circuit breaker at 14% (1 pt buffer below the 15% user target).
    // Was 20%, which allowed breaches past the stated limit before firing.
    // Override via env DRAWDOWN_LIMIT_PCT if needed.
